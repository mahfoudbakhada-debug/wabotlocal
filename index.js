...
function formatHuecosPro(lista){
  return lista.map(h => `• ${h}`).join('\n');
}

app.get('/health',(req,res)=>res.send('OK'));

// --- LANDING V5 LILA ORO 500K (IGUAL) ---
app.get('/', (req,res)=>{
...
// --- BOT PRO - MISMO INDEX + MEMORIA + HABLA PREMIUM ---
app.post('/whatsapp', async (req,res)=>{
  ...
  try{
    ...
    const tieneDia = body.includes('lunes')||body.includes('martes')||body.includes('miércoles')||body.includes('miercoles')||body.includes('jueves')||body.includes('viernes')||body.includes('sábado')||body.includes('sabado')||body.includes('mañana')||body.includes('hoy')||body.includes('domingo')||body.includes('pasado');
    const tieneHora = hora!==null;

    // FIX MEMORIA: Si estamos esperando hora y solo dice hora, NO pisar la fecha que ya tenemos
    if(ses.estado==='pidiendo_hora' && tieneHora &&!tieneDia && ses.fecha){
      ...
      if(!libres.includes(hora+':00')){
        ...
        reply=`Perfecto, he bloqueado el ${diaCap2} a las ${hora}:00 para ti ${EMOJI}\n\n¿A nombre de quién hago la reserva? Por favor, nombre y apellidos.`;
      }
    }
      ...
      const diaCap = ses.fecha.toLocaleDateString('es-ES',{weekday:'long',day:'numeric',month:'long'});
      const diaCap2 = diaCap.charAt(0).toUpperCase()+diaCap.slice(1);
      reply=`Para el ${diaCap2} tengo libres:\n\n${formatHuecosPro(libres)}\n\nDime la hora con formato 10:00 por ejemplo, y te la reservo.`;
    }
    else if(tieneDia || tieneHora){
      // Si dice nuevo dia, actualizar memoria
    ...
    else if(ses.estado==='inicio' || body.match(/^(hola|buenas|hello|ola|hey|buenas tardes|buenos dias)$/)){
      ...
      if(raw.split(' ').length < 2){
        reply=`¿Me indicas tu nombre completo por favor? ${EMOJI}\n\nNecesito nombre y apellido para confirmar la reserva.\n\nEjemplo: Ana García`;
      } else {
        ...
        reply=`¡Reservado, ${ses.nombre.split(' ')[0]}! ${EMOJI}\n\nTu cita en ${BUSINESS_NAME} está confirmada:\n\n📅 ${diaCap}\n👤 ${ses.nombre}\n📍 ${BUSINESS_ADDRESS}\n\nSi necesitas cambiar o cancelar, escríbeme por aquí.\n\n¡Gracias por confiar en nosotros!`;
        delete sesiones[from];
      }
    } else {
      reply=`¿Me ayudas con el día y la hora? ${EMOJI}\n\nEjemplo: "Mañana a las 10:00" o "El viernes a las 17:00"`;
    }
  }catch(e){ console.log(e); reply=`Disculpa, no te he entendido bien. ¿Me repites el día y la hora por favor? ${EMOJI}`; }

  res.set('Content-Type','text/xml');
  res.send(`<Response><Message>${reply}</Message></Response>`);
});

app.listen(PORT,()=>console.log('LIVE V5 PRO MEMORIA '+BUSINESS_NAME));
