...
function formatHuecos(list){ return list.map(h=>`• ${h}`).join('\n'); }
...
// --- BOT PRO MEJORADO - MISMO INDEX BUENO ---
  ...
  try{
    ...
    if(tieneDia || tieneHora){
      ...
      if(hora===null){
        ...
        const diaCap = diaTxt.charAt(0).toUpperCase()+diaTxt.slice(1);
        reply=`Para el ${diaCap} tengo libres:\n\n${formatHuecos(libres)}\n\nDime la hora por favor, ejemplo: 10:00`;
      } else {
        ...
        if(!libres.includes(hora+':00')){
          reply=`Esa hora ya está ocupada ${EMOJI}\n\nPara el ${diaCap} me quedan:\n\n${formatHuecos(libres)}\n\n¿Cuál prefieres?`;
        } else {
          ses.estado='pidiendo_nombre';
          reply=`Perfecto, he bloqueado el ${diaCap} a las ${hora}:00 para ti ${EMOJI}\n\n¿A nombre de quién hago la reserva?\nPor favor, nombre y apellidos.`;
        }
      }
    }
...
app.listen(PORT,()=>console.log('LIVE V5 PRO FINAL '+BUSINESS_NAME));
